import React, { useState, useRef, useEffect, useCallback } from 'react'; // <-- Add useCallback here
...
const OrderAssistant = () => {
  ...
  // Function to send message to the LLM - now memoized with useCallback
  const handleSendMessage = useCallback(async (messageToSend = userInput) => {
  ...
  }, [chatHistory, userInput, isRecording, setChatHistory, setUserInput, setIsLoading]); // <-- Dependencies for useCallback
  ...
  useEffect(() => {
    if ('webkitSpeechRecognition' in window) {
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        handleSendMessage(transcript); // <-- Using the memoized function
      };
  ...
  }, [handleSendMessage, setChatHistory, setUserInput, setIsLoading]); // <-- Add handleSendMessage and other setState functions here
  ...
  const toggleRecording = () => {
    if (isRecording) {
      ...
      setChatHistory(prev => prev.filter(msg => msg.text !== 'Listening...')); // Remove old "Listening..."
